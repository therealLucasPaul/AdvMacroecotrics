...
yoy_INDPRO <- ts(industrial_prod$yoy_growth, frequency = 12, start = c(1959,1,1))
...
```
\begin{itemize}
\item Using the the packages vars in R (or an equivalent one in another language), estimate the VAR described in section 2.2 using the variables in the same order as specified by Kilian & Park (2009).
```{r}
...
\item Calculate forecast error variance decompositions for the included variables (take a look at the fevd() function in vars). Replicate Table 2 of Kilian & Park (2009). Interpret the results.
